...
### Removed
- **Dropped the Gitea dev-build workflow entirely** (and its Gitea-specific job guards in the GitHub workflow), removing the last hardcoded internal address from the repository. CI now targets GitHub / ghcr.io only.
...
### Removed
- **Internal dead-code cleanup** - Removed unused code with no user-facing effect: two never-wired livestream `detect-clean` API endpoints (the UI only ever exposed detect-clean for Shorts), a leftover `scan_all_channels` alias, and two unreferenced helper methods (`_subtitles_enabled`, `_get_max_duration`). Reduces confusion for future maintenance.
...
### Improved
- **More complete Rumble channel art** (#19) - Channel scraping now also pulls the channel's banner image (the header backsplash, separate from the avatar), so the channel detail page hero banner is populated. The description is left empty when a channel's page title is just its name rather than a real tagline, instead of duplicating the name.
...
### Documentation
- Updated the README to reflect the player client changes: added a "Player Client Selection" feature note, a `FORMAT_UNAVAILABLE` error-table row, and a troubleshooting entry for "Requested format is not available". Corrected the download pipeline description (player client is now auto-selected by default) and added 4K/2160p to the per-channel quality options.
...
### Fixed
- **Private/deleted videos showing in playlists** - When scanning a YouTube playlist, private, deleted, and unavailable videos were stored as real entries with placeholder titles like "[Private video]". These are now skipped during scans, and the number skipped is logged.
- **Cookie watcher error spam** - When `/cookies` and `/config` are mounted to overlapping paths, the cookie importer logged a "same file" error every interval. It now detects this case and skips quietly.
...
- **Sonarr-style episode management UI** - Major overhaul of the channel detail page with a cleaner, more functional layout:
  ...
  - **Total channel size** - Header stats now show total disk usage for the channel
...
### Changed
- **Version reset** - Consolidated all prior 1.4.x through 1.7.x releases into a single clean v1.8.0 release. No functional changes from v1.7.39.
...
### Fixed
- **Force-rescan still queues matched files for download** - After force-rescan matched existing files on disk back to video records, the videos were marked as completed but their download queue entries were not removed. The download worker would then re-download them anyway. Queue entries are now cleaned up for all matched files. (#14)

## [1.7.37] - 2026-04-27

### Fixed
- **Force-refresh creates duplicate downloads** - After force-rescan deletes and recreates video records, existing MP4 files on disk are now matched back to prevent re-downloads. Matches by video ID in the filename first, then falls back to reading `.info.json` sidecar files for custom naming templates that don't include `{video_id}`. (#14)

## [1.7.35] - 2026-04-27

### Fixed
- **Channels page limited to 50 channels** - The channels list API defaulted to returning 50 results and the frontend didn't override it. Raised the default to 500 (max 1000). (Fixes #13)
...
### Fixed
- **Missing UI for chapters and Discord** - The chapter embedding toggle and Discord webhook URL input were added to the backend in v1.7.32 but not wired into the settings UI. Now visible under Settings > Media Management and Settings > Connect respectively.
