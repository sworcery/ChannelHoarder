...
STANDALONE_CHANNEL_ID = "__standalone__"  # Legacy channel ID for migration
...
async def _get_or_create_uploader_channel(
    db: AsyncSession, info: dict, url: str, download_dir: Optional[str] = None,
) -> Channel:
    """Get or create a channel for the video's uploader.

    Uses the uploader/channel info from yt-dlp to create a proper channel
    record so standalone downloads are organized by uploader in the main
    download directory.
    """
    from app.utils.platform_utils import detect_platform

    # Extract uploader identity from yt-dlp info
    uploader_id = info.get("channel_id") or info.get("uploader_id") or ""
    uploader_name = info.get("channel") or info.get("uploader") or "Unknown"
    channel_url = info.get("channel_url") or info.get("uploader_url") or ""
    thumbnail_url = info.get("channel_thumbnail") or info.get("thumbnail")
    platform = detect_platform(url)

    if not uploader_id:
        # Fallback: use sanitized uploader name as ID
        from app.utils.file_utils import sanitize_filename
        uploader_id = f"standalone_{sanitize_filename(uploader_name).replace(' ', '_').lower()}"

    # Check if channel already exists
    result = await db.execute(
        select(Channel).where(Channel.channel_id == uploader_id)
    )
    channel = result.scalar_one_or_none()
    ...
    # Create new channel for this uploader
    channel = Channel(
        ...
        quality="best",
        enabled=False,  # Don't auto-scan standalone uploader channels
        auto_download=False,
        health_status="healthy",
        download_dir=download_dir,  # None = use global /downloads default
    )
    db.add(channel)
    await db.commit()
    await db.refresh(channel)
    logger.info("Created uploader channel for standalone download: %s (%s)", uploader_name, uploader_id)
    return channel
...
async def download_standalone_video(
    ...
    """Download a standalone video by URL, organized under the uploader's channel."""
    ...
    # Get or create a channel for this video's uploader
    channel = await _get_or_create_uploader_channel(db, info, body.url, body.download_dir)
    ...
    logger.info("Standalone download queued: %s (%s) -> channel '%s'", video.title, video_id, channel.channel_name)
    return {
...
@router.post("/standalone/reorganize", status_code=200)
async def reorganize_standalone_downloads(db: AsyncSession = Depends(get_db)):
    """Migrate videos from the legacy '__standalone__' channel to proper per-uploader channels.

    For each video in the old standalone channel, fetches uploader info from yt-dlp,
    ...
    # Find legacy standalone channel
    result = await db.execute(
        select(Channel).where(Channel.channel_id == STANDALONE_CHANNEL_ID)
    )
    old_channel = result.scalar_one_or_none()
    if not old_channel:
        return {"message": "No legacy standalone channel found", "migrated": 0}

    # Get all videos in the legacy channel
    result = await db.execute(
        select(Video).where(Video.channel_id == old_channel.id)
    )
    videos = result.scalars().all()

    if not videos:
        # No videos, just delete the old channel
        await db.delete(old_channel)
        await db.commit()
        return {"message": "Legacy standalone channel was empty, removed", "migrated": 0}
    ...
    for video in videos:
        try:
            # Fetch uploader info from yt-dlp
            video_url = build_video_url("youtube", video.video_id)
            info = await asyncio.to_thread(ytdlp.get_video_info_by_url, video_url)

            if not info:
                logger.warning("Could not fetch info for %s, skipping migration", video.video_id)
                failed += 1
                continue

            # Create or find the uploader's channel
            new_channel = await _get_or_create_uploader_channel(db, info, video_url)

            # Calculate new episode number
            episode_count = await db.scalar(
                select(func.count(Video.id))
                .where(Video.channel_id == new_channel.id)
                .where(Video.season == video.season)
            )
            new_episode = (episode_count or 0) + 1

            # Build new file path
            old_path = video.file_path
            if old_path and video.status == "completed":
                new_path_base = build_output_path(
                    ...
                    season=video.season,
                    episode=new_episode,
                    naming_template=new_channel.naming_template,
                    base_dir=new_channel.download_dir,
                )
                ext = os.path.splitext(old_path)[1] if old_path else ".mp4"
                new_path = new_path_base + ext
            ...
            video.channel_id = new_channel.id
            video.episode = new_episode
            migrated += 1

        except Exception as e:
            logger.warning("Failed to migrate standalone video %s: %s", video.video_id, e)
            failed += 1

    # Delete the old standalone channel if all videos were migrated
    remaining = await db.scalar(
        select(func.count(Video.id)).where(Video.channel_id == old_channel.id)
    )
    if remaining == 0:
        await db.delete(old_channel)
        logger.info("Deleted legacy standalone channel")

    await db.commit()

    return {
        "message": f"Migrated {migrated} standalone videos to uploader channels ({failed} failed)",
        "migrated": migrated,
        "failed": failed,
    }
...
async def get_standalone_settings(db: AsyncSession = Depends(get_db)):
    """Get the current standalone download directory (legacy support)."""
    result = await db.execute(
        select(Channel).where(Channel.channel_id == STANDALONE_CHANNEL_ID)
    )
    channel = result.scalar_one_or_none()
...
async def update_standalone_settings(
    ...
    """Update the standalone download directory (legacy support)."""
    try:
        validate_download_path(body.download_dir, app_settings.allowed_download_roots)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))

    # This only affects the legacy standalone channel if it exists
    result = await db.execute(
        select(Channel).where(Channel.channel_id == STANDALONE_CHANNEL_ID)
    )
    channel = result.scalar_one_or_none()
    if channel:
        channel.download_dir = body.download_dir
        await db.commit()
    return {"download_dir": body.download_dir, "message": "Standalone download directory updated"}
